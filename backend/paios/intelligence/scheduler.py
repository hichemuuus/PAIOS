...
from paios.intelligence.models.registry import get_registry
from paios.intelligence.training.dataset import TrainingDataset, load_user_interactions
    ...
    async def _cycle(self) -> None:
        ...
        try:
            result = await asyncio.to_thread(
                self._orchestrator.promote_if_better, dataset, "intent_classifier"
            )
            if result.success and result.promoted:
                logger.info("scheduler: promoted intent_classifier v%s", result.metadata.version)
        except Exception as e:
            logger.warning("scheduler: intent_classifier retrain failed: %s", e)

        try:
            result = await asyncio.to_thread(
                self._orchestrator.promote_if_better, dataset, "tool_selector"
            )
            if result.success and result.promoted:
                logger.info("scheduler: promoted tool_selector v%s", result.metadata.version)
        except Exception as e:
            logger.warning("scheduler: tool_selector retrain failed: %s", e)
