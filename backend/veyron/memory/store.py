...
class MemoryStore:
    ...
    def search(
        ...
        + importance + recency. If ``use_reranker`` is True and a trained
        MemoryRetrievalModel is available, the top candidate pool is reranked
        by the model before returning final results.

        This is a safe two-stage pipeline:
        1. Heuristic candidate retrieval (keyword + quality scores)
        ...
        # Keep a generous candidate pool for reranking (at least top 20 or 4x limit).
        candidate_pool = scored[:max(limit * 4, 20)]
